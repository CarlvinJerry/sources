...
## Influencing online User decisions on Twitter through nudge units  
### Introduction  
Here is a fun fact; An average human being (probably an adult) makes close to 30,000 conscious decisions every day. This isn't entirely true though, in fact, I just made that number up. I could be right because if you think about it, how many decisions would you say you make on a day to day basis? Depending on who you are the above obviously varies widely and you know best. We all make __**n**__ decisions every day- what to do, eat, buy or hit. The real question however is, do we always choose well? Are there any other factors at hand that influence our decision making process? Are all these factors, if any, always straight forward choices or do we sometimes get __"nudged"__ into these choices we make?    
...
<figure>
  <img  = src = "https://carlvinjerry.github.io/post/MyImages/theChallenge.jpg", width = "500", height = "433">
...
**Note:** To keep this post concise, code for the workings has been minimized. Click on the **"Code"** buttons to expand code chunks for anyone interested in trying out the same process. The code is well commented for easier understanding as well.
...
```{r Fetching twitter data, eval=FALSE, tidy=TRUE}
...
A total of **7000 tweets** were captured. The data frame had a total of 88 columns which we treated as variables for the research. However, not all variables were used in the research we therefore had to do some data cleaning.open the code chunk to view the variables in our raw data.  

```{r, Column names preview from the raw data,eval=FALSE, tidy=TRUE}
> bind_Data<- as.data.frame(bind_Data)
> names(bind_Data)
...
This stage involved cleaning up our data by removing the unwanted columns/variables. We decided to do with a select few variables we thought would be most appropriate for our case study. We chose the following **seven variables:**
  ...
  * **Verified** - whether or not the user is a verified on twitter.
...
```{r, data sorting and variable selection,eval=FALSE, tidy=TRUE }
...
```{r, Data import,eval=FALSE,tidy=TRUE}
> #Data Prep
> #Shuffle data
...
```{r, data prep,eval=FALSE,tidy=TRUE}
#Clean Data
library(dplyr)
# Drop variables
clean_data<- data %>%
...
```{r, Split train and test data sets,eval=FALSE,tidy=TRUE}
#Train & Test sets
...
```{r, model fit,eval=FALSE,tidy=TRUE}
...
<figure>
  <img  = src = "https://carlvinjerry.github.io/post/MyImages/tagged.png", width = "503", height = "433">
...
```{r, prediction accuracy,eval=FALSE,tidy=TRUE}
 #Predicting
...
<figure>
  <img  = src = "https://carlvinjerry.github.io/post/MyImages/math.jpg">
  <figcaption>Figure 3: The formula</figcaption>
</figure>
