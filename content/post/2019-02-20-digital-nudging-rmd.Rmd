...
>**Note:** To keep this post concise, code for the workings has been minimized. [The source code for this post can be found herer,](https://github.com/CarlvinJerry/sources/blob/master/content/post/2019-02-20-digital-nudging-rmd.Rmd) for anyone interested in trying out the same process. The code is well commented for easier understanding as well.
...
```{r Fetching twitter data, eval=FALSE, include=FALSE, tidy=TRUE}
...
```{r Column names preview from the raw data, eval=FALSE, eval=FALSE, include=FALSE, tidy=TRUE}
> bind_Data<- as.data.frame(bind_Data)
> names(bind_Data)
...
```{r data sorting and variable selection, eval=FALSE, eval=FALSE, include=FALSE, tidy=TRUE}
...
```{r Data import,eval=FALSE, eval=FALSE, include=FALSE, tidy=TRUE}
> #Data Prep
> #Shuffle data
...
```{r data prep,eval=FALSE, eval=FALSE, include=FALSE, tidy=TRUE}
#Clean Data
library(dplyr)
# Drop variables
clean_data<- data %>%
...
```{r Split train and test data sets,eval=FALSE, include=FALSE, eval=FALSE, tidy=TRUE}
#Train & Test sets
...
```{r model fit, eval=FALSE, include=FALSE, eval=FALSE, tidy=TRUE}
...
```{r prediction accuracy, eval=FALSE, include=FALSE,eval=FALSE, tidy=TRUE}
 #Predicting
